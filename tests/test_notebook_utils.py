...
class TestParentBackUrl:
    ...
    def test_reads_local_parent_directory(self, tmp_path):
        (tmp_path / "lessons.tsv").write_text(self._PARENT_TSV, encoding="utf-8")
        child_dir = tmp_path / "child"
        child_dir.mkdir()
        with patch("urllib.request.urlopen", side_effect=AssertionError("should not hit network")):
            result = parent_back_url(str(child_dir / "notebook.py"), "https://example.com/parent/lessons.tsv")
        assert result == "https://molab.marimo.io/notebooks/nb_PARENT/app"

    def test_falls_back_to_remote_when_no_local_parent(self, tmp_path):
        child_dir = tmp_path / "child"
        child_dir.mkdir()
    ...
    def test_empty_parent_returns_none(self, tmp_path):
        child_dir = tmp_path / "child"
        child_dir.mkdir()
    ...
    def test_repeat_call_is_cached_no_second_fetch(self, tmp_path):
        child_dir = tmp_path / "child_cache_test"
        child_dir.mkdir()
        _path, _url = str(child_dir / "notebook.py"), "https://example.com/cache-test/lessons.tsv"
        with patch("urllib.request.urlopen", return_value=_make_resp(self._PARENT_TSV.encode("utf-8"))) as _mock:
            parent_back_url(_path, _url)
            parent_back_url(_path, _url)
