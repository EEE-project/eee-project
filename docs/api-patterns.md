...
the parent lookup — it fetches the parent's own `lessons.tsv` (local-first,
network fallback on molab) and returns its `index_url`:
...
eee_topbar(mo, back_url=back_url, lang=lang_sel.value, titles="Kapodistrias", style="index")
```
