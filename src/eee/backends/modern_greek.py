    ...
    def inflect(self, lemma: str, features: dict[str, str], pos: str) -> set[str]:
        ...
        if pos == "verb":
            ...
            if gender_path is None:
                # No gender specified — union over known gender keys in the paradigm
                dummy_path = mg_noun_path({**features, "Gender": "Masc"})
                rest_path = dummy_path[1:]  # [number_key, case_key]
                result: set[str] = set()
                for gender_key in _GENDER_KEYS:
                    if gender_key in full_paradigm:
                        result |= _walk(full_paradigm[gender_key], rest_path)
                return result
