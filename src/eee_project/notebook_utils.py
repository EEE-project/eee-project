...
def eee_topbar(mo, back_url: str, lang: str, titles: "dict | str", *, style: str = "back", icon: str = "●", ga_config=None):
    ...
    if style == "index":
        left = (f'<a class="tb-back" href="{back_url}" target="_blank" rel="noopener">{icon} {title}</a>'
...
def parent_back_url(path, parent_url: str, *, timeout: int = 5) -> "str | None":
    """Resolve ``eee_topbar``'s ``back_url`` for a course/grouping-index page.

    Fetches the *parent* index's own ``lessons.tsv`` — local-first from the
    parent directory (real checkout / local dev), falling back to
    ``parent_url`` over the network (molab, which only bundles the calling
    notebook's own directory) — and returns its ``index_url``. Every row in
    a grouping-level ``lessons.tsv`` should carry the same value: the parent
    page's own URL, repeated per row exactly like lesson-level ``index_url``
    already is one level down.

    Cached per ``(path, parent_url, timeout)`` for the life of the kernel —
    ...
    Args:
        path:       Pass ``__file__`` from the child index notebook.
        parent_url: Remote URL of the parent's ``lessons.tsv``.
        timeout:    Network timeout in seconds for the remote fallback.
    ...
    Example cell (course index one level below a grouping index)::
        ...
        back_url = parent_back_url(
            __file__, f"{_ROOT}/modern_greek/b1greeklanguageandculture/lessons.tsv",
        )
        eee_topbar(mo, back_url=back_url, lang=lang, titles=TITLES, style="index")
    """
    from pathlib import Path as _Path

    parent_dir = _Path(path).parent.parent
    return ConfigStore.from_file_or_url(parent_dir, parent_url, timeout=timeout).index_url()
...
def make_paradigm_form(mo, labels, values=None):
    ...
    Pressing Enter in any field flushes the current values immediately (no
    debounce wait), records the field's index in ``.widget.enter_field_index``,
    and increments ``.widget.submit_count`` — treat that counter as an
    additional "check" trigger alongside a submit button, the same way the
    button's own click counter is watched.
